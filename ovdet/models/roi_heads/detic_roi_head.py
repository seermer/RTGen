...
class DeticRoIHead(CascadeRoIHead):
    ...
    def run_ovd(self, x, batch_data_samples, rpn_results_list, ovd_name, batch_inputs, *args, **kwargs) -> dict:
        ...
        labels = [None] * len(batch_data_samples)
        ...
        for stage in range(self.num_stages):
            ...
            sampling_results = [ovd_method.sample(res, data_sample, stage=stage)
                                for res, data_sample in zip(results_list, batch_data_samples)]
            for sampling_result in sampling_results:
                sampling_result.bboxes = sampling_result.bboxes.detach()

            labels = [sampling_result.labels for sampling_result in sampling_results]
            ...
            if stage < self.num_stages - 1:
                ...
                with torch.no_grad():
                    if 'region' in ovd_name:
                        labels = torch.concat(labels)
                        non_gts = labels < 0
                        bbox_results = {k: v[non_gts] for k, v in bbox_results.items()}
                    if len(bbox_results['rois']) > 0:
